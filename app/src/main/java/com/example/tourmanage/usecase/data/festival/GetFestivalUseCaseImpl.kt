...
import com.example.tourmanage.toFestivalItems
...
class GetFestivalUseCaseImpl @Inject constructor(
    ...
    override suspend fun invoke(areaCode: String, startDate: String): Result<Flow<ArrayList<FestivalItem>>> = runCatching{
        flow {
            var date = startDate
            if (date.isEmpty()) {
